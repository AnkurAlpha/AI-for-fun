...
# number of heads and number of layers
n_head = 8
n_layer = 4
...
class FeedForward(nn.Module):
    # here each token is processed independentyly
    # while in self-attention tokens talk to each other
    def __init__(self, C: int):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(C, 4*C),  # expand
            nn.ReLU(),          # make non-linear
            nn.Linear(4*C, C)   # again compress
        )
...
class Block(nn.Module):
    def __init__(self, C: int, n_head: int, block_size: int):
        ...
        self.attn = MultiheadAttention(C, n_head, block_size)
        self.ff = FeedForward(C)

    def forward(self, x):  # x shape : B,T,C
        x = x + self.attn(self.ln1(x))  # attention + residual
        x = x + self.ff(self.ln2(x))    # MLP + residual
        return x
...
class AttentionLLM(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_dimension: int):
        ...
        self.blocks = nn.Sequential(*[
            Block(embedding_dimension, n_head=n_head, block_size=block_size)
    ...
    def forward(self, x, targets=None):
        ...
        h = self.ln_f(h)  # final normalization
