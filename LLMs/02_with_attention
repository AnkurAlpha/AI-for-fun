...
class AttentionLLM(nn.Module):
    ...
    def __init__(self, vocab_size: int, embedding_dimension: int):
        ...
        # Implementing Single head causal self attention(C=embedding_dimension)
        self.key = nn.Linear(embedding_dimension,
                             embedding_dimension, bias=False)
        self.value = nn.Linear(embedding_dimension,
                               embedding_dimension, bias=False)
        self.query = nn.Linear(embedding_dimension,
                               embedding_dimension, bias=False)
    ...
    def forward(self, x, targets=None):
        ...
        # dim=0 would normalize across batches (wrong)
        # dim=1 would normalize across query positions (wrong)
        # dim=-1 normalizes across key positions (correct)

        # weighted sum of values: (B,T,T) @ (B,T,C) -> (B,T,C)
        att_out = weights @ v

        # now produce logits
        logits = self.lm_head(att_out)  # (B,T,V)
        # takes C-dim vector and outputs a vocab size - dimensional vector
        # using positional embeddings

        # Meaning: for one token representation (lenght C), produce scores for
        # every possible next token (length V)
        # C = embedding_dimension , V = vocab_size(30,000)
        loss = None
        ...
        print(f"Tokennizer decode from forward(): {
              tokenizer.decode(top.indices.tolist())}")
